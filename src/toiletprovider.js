...
ToiletProvider = function () {
  var that = this;
  mongodbUri =
    process.env.PROD_MONGODB /*MONGOLAB_URI*/ ||
    "mongodb://localhost/kktoiletslocater_dev";
...
ToiletProvider.prototype.findByType = function (
  page,
  type_query,
  district_query,
  time_query,
  ...
  var toiletCollectionQuery = function (error, type_results) {
    thisToiletProvider.getCollection(function (error, toilet_collection) {
      if (error) callback(error);
        ...
        if (district_query) query_obj["properties.bydel"] = district_query;
        if (time_results.length > 0 && time_query)
          query_obj["properties.toilet_aaben_tid"] = {
            $in: time_results_filtered,
          };
        var paged_query = function (res_per_page, res_pages) {
          var pages_count = res_pages;
          toilet_collection
            .find(query_obj, { type: 0 })
            .skip(res_per_page * page)
            .limit(res_per_page)
            .toArray(function (error, results) {
              if (error) callback(error);
  ...
  var toiletTypeQuery = function (error, time_results_cb) {
    ...
    thisToiletProvider.getTypeCollection(function (
      error,
      toilet_type_collection
    ) {
      toilet_type_collection
        .find({ gruppe: type_query }, { gruppe: 0, _id: 0 })
        .toArray(toiletCollectionQuery);
    });
  };
  this.getTimeCollection(function (error, toilet_time_collection) {
    toilet_time_collection
      .find({ gruppe: time_query }, { gruppe: 0, _id: 0 })
      .toArray(toiletTypeQuery);
  });
};
...
ToiletProvider.prototype.typeSelector = function (callback) {
  this.getTypeCollection(function (error, toilet_types_collection) {
    if (error) callback(error);
      ...
      toilet_types_collection
        .aggregate([{ $group: { _id: "$gruppe" } }, { $sort: { _id: 1 } }])
        .toArray(function (error, results) {
          if (error) callback(error);
          else {
            callback(null, results);
...
ToiletProvider.prototype.findById = function (id, callback) {
  this.getCollection(function (error, toilet_collection) {
    if (error) callback(error);
      ...
      toilet_collection.findOne(
        {
          _id: toilet_collection.db.bson_serializer.ObjectID.createFromHexString(
            id
          ),
        },
